...
int main(void) {
  // Inicializamos una ventana de hd p�xeles con un t�tulo personalizado
  ...
  // enemigo
  Enemigo enemigo(&texEnemigo, 500, 300, // posici�n inicial
                  60, 60,                // ancho - alto
                  150.0f,                // velocidad vertical
                  200.0f,                // l�mite arriba
                  600.0f                 // l�mite abajo
  );
  ...
  PlataformaNube plataformas[] = {
      ...
      // PlataformaNube(&texturaNube, 400, 400, 120, 40, WHITE), //desactivada
      // para darle dificultad pero lo activo para probar ganar
  ...
  while (!WindowShouldClose()) {
    ...
    if (!perdiste && !ganaste) {
      ...
      jugador.actualizar(dt, piso, plataformas, cantPlataformas, rect.getRect(),
                         sonidoSalto);
    ...
    // insertamos la imagen de fondo me guie con la ia para poder hacerlo a
    // escala y que se vea bien porque no me salia del tama�o de la ventana
    ...
    // dibujo tesoro
    if (!ganaste) {
      DrawTexturePro(texTesoro,
                     {0, 0, (float)texTesoro.width, (float)texTesoro.height},
                     tesoroRect, {0, 0}, 0.0f, WHITE);
    }

    // Dibujamos textos informativos en pantalla
    DrawText(
        TextFormat("Resolucion: %dx%d", GetScreenWidth(), GetScreenHeight()),
        10, 10, 20, BLACK);
    DrawText("mover-> D, mover <- A, Espacio salta, P para salir, R reiniciar ",
             250, 10, 20, BLACK);
    DrawText("El piso es lava! El tesoro gana! ", 10, 70, 20, BLACK);
    DrawText(TextFormat("Jugador X: %.1f Y: %.1f", jugador.getRect().x,
                        jugador.getRect().y),
             10, 40, 20, BLACK);
